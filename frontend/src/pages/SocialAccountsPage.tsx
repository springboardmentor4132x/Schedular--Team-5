...
export function SocialAccountsPage() {
  ...
  const handleConnect = async (platformId: string) => {
    try {
      ...
      if (platformId === 'facebook') {
        response = await accountService.facebookLogin();
      } else if (platformId === 'instagram') {
        response = await accountService.instagramLogin();
      } else {
        setError(
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      await accountService.delete(disconnectTarget);

      setAccounts((prev) =>
        prev.filter(
          (account) => account.id !== disconnectTarget
        )
      );
