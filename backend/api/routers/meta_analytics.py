...
def _extract_meta_error(detail) -> Dict:
...
# ============================================================
# INSTAGRAM DEMOGRAPHIC HELPERS
# ============================================================


def _extract_demographic_results(
    response: Dict,
    breakdown: str,
) -> list:
    """
    Extract Instagram follower demographic results from
    Meta's total_value.breakdowns.results structure.
    ...
    for insight in response.get("data", []):
        ...
        for breakdown_item in breakdowns:

            dimension_keys = breakdown_item.get(
                "dimension_keys",
                [],
            )
            ...
            raw_results = breakdown_item.get(
                "results",
                [],
            )
...
def _format_instagram_demographic_response(
    response: Dict,
    breakdown: str,
) -> Dict[str, Any]:
    ...
    extracted_results = _extract_demographic_results(
        response,
        breakdown,
    )
    ...
    return {
        ...
        "raw_insights": raw_data,
    }


async def _get_instagram_demographic_breakdown(
    ig_user_id: str,
    access_token: str,
    breakdown: str,
) -> Dict[str, Any]:
    """
    Fetch one Instagram follower-demographic breakdown.
    ...
    try:
        ...
        formatted = _format_instagram_demographic_response(
            response,
            breakdown,
        )
        ...
        return {
            ...
            "breakdown": breakdown,
            "data": formatted["data"],
            "raw_insights": formatted["raw_insights"],
        }
...
@router.get("/facebook/{account_id}/geography")
async def get_facebook_geography(
    account_id: str,
    db: Annotated[Session, Depends(get_db)],
) -> Dict:
    ...
    return {
        "platform": "FACEBOOK",
        "report_type": "AUDIENCE_GEOGRAPHY",
        "data": _format_insights(data),
        "errors": errors,
    }
...
@router.get("/instagram/{account_id}/demographics")
async def get_instagram_demographics(
    ...
    for breakdown in breakdowns:
        ...
        result = await _get_instagram_demographic_breakdown(
            ig_user_id,
            access_token,
            breakdown,
        )
    ...
    # If Meta returned no rows at all, provide a useful
    # explanation instead of silently returning empty data.
    if (
        not any(
            item.get("data")
            for item in results
        ...
        errors.append(
            {
                "message": (
                    "Meta returned no follower demographic "
                    "results for this Instagram account."
                ),
                "possible_reason": (
                    "Instagram demographic insights may require "
                    "sufficient audience data. Meta commonly "
                    "requires at least 100 followers for "
...
@router.get("/instagram/{account_id}/geography")
async def get_instagram_geography(
    account_id: str,
    db: Annotated[Session, Depends(get_db)],
) -> Dict:
    ...
    if (
        ...
        errors.append(
            {
                "message": (
                    "Meta returned no follower geography "
                    "results for this Instagram account."
                ),
                "possible_reason": (
                    "Instagram audience geography insights may "
                    "require sufficient audience data. Meta "
                    "commonly requires at least 100 followers "
                    "for audience demographic breakdowns."
                ),
                "timeframe": "last_90_days",
            }
        )
