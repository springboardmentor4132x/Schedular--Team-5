...
# Import your real and mock Analytics endpoints
from api.routers.analytics import (
    get_youtube_audience_analytics,
    get_youtube_performance_trends,
    get_linkedin_audience,
    get_linkedin_trends,
    get_x_audience,  # Added X Mock
    get_x_trends,  # Added X Mock
    get_pinterest_audience,  # Added Pinterest Mock
    get_pinterest_trends  # Added Pinterest Mock
)
...
async def fetch_real_analytics_data(
    ...
    """Aggregates real data from the database and Analytics endpoints."""
    # Added x and pinterest to the initial data dictionary
    data = {
        "youtube": {"posts": 0, "impressions": 0, "clicks": 0, "followers": 0, "top_demo": "N/A", "roi": "N/A"},
        "linkedin": {"posts": 0, "impressions": 0, "clicks": 0, "followers": 0, "top_demo": "N/A", "roi": "N/A"},
    ...
    # 1. Get real post counts for all platforms
    data["youtube"]["posts"] = db.query(Schedule).join(SocialAccount).filter(
        SocialAccount.platform == "youtube", Schedule.status == ScheduleStatus.PUBLISHED).count()
...
async def export_platform_excel(
    ...
    # Updated validation to include X and Pinterest
    if platform not in ["youtube", "linkedin", "x", "pinterest"]:
...
async def export_platform_pdf(
    ...
    # Updated branding to include X (Black) and Pinterest (Crimson Red)
    if platform == "youtube":
