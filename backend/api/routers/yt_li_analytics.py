...
async def get_youtube_channel_content(
    ...
    The endpoint:
    1. Gets the authenticated YouTube channel.
    2. Gets the channel's uploads playlist.
    3. Gets recent videos from that playlist.
    4. Gets statistics for those videos.
    5. Returns frontend-friendly content data.
    """
    ...
    async with httpx.AsyncClient(
        ...
        if videos_response.status_code != 200:
            ...
            raise HTTPException(
                ...
                detail=(
                    "Failed to fetch YouTube video statistics"
                )
            )
...
async def get_youtube_performance_trends(
    ...
    """
    Fetches a historical trend report for the channel.
    """
...
async def get_youtube_geography(
    ...
    """
    Fetches geographic distribution
    (views by country).
    """
    ...
    async with httpx.AsyncClient(
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(
                status_code=response.status_code,
                detail=(
                    "Failed to fetch geography stats"
                )
            )
    ...
    async with httpx.AsyncClient(
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(
                status_code=response.status_code,
                detail=(
                    "Failed to fetch demographics stats"
                )
            )
...
async def get_linkedin_audience_mock(
    ...
    """
    Mocks follower statistics for a
...
async def get_linkedin_trends_mock(
    ...
    """
    Mocks a historical trend report for LinkedIn.
    """
...
async def get_linkedin_demographics_mock(
    ...
    """
    Mocks audience demographics
    (seniority and industry) for LinkedIn.
    """
