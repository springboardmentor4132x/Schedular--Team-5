...
from api.services.user import get_users, add_user, login_user, update_profile, change_password, delete_user
...
def list_all_users(current_user=Depends(require_role("administrator", "marketing_team"))):
    return get_users()


@router.delete("/{user_id}")
def delete_user_account(user_id: int, current_user=Depends(require_role("administrator"))):
...
def marketing_route(current_user=Depends(require_role("marketing_team", "administrator"))):
    return {"message": "Welcome Marketing Team", "user": current_user}


@router.get("/business")
def business_route(current_user=Depends(require_role("business_user", "administrator"))):
    return {"message": "Welcome Business User", "user": current_user}


@router.get("/creator")
def creator_route(current_user=Depends(require_role("content_creator", "administrator"))):
    return {"message": "Welcome Content Creator", "user": current_user}